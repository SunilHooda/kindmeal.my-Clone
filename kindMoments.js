let product = [{
    logo: "https://www.kindmeal.my/photos/member/25/25560-s.jpg",
    name: "LimKengTiong",
    date: "Jun 14th 2022",
    image: ["https://www.kindmeal.my/photos/moment/24/24024-45787-m.jpg",

        "https://www.kindmeal.my/photos/moment/24/24024-45786-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24039-45827-m.jpg"

    ],
    socialLogo: "https://www.kindmeal.my/images/icon_facebook_moment.png",
    link: "GEKIL",
    address: "no. 49-g jalan pju 5/20 the strand, 47810 Petaling Jaya",
    likeimg: "https://www.kindmeal.my/images/icon_heart_darkgrey.svg",
    camara: "https://www.kindmeal.my/images/icon_camera_darkgrey.png",
    msg: "https://www.kindmeal.my/images/icon_comment_darkgrey.png",
    des: "鸳鸯火锅（麻辣+番茄汤底） 北京烤鸭",

},
{
    logo: "https://www.kindmeal.my/photos/member/11/11540-s.jpg",
    name: "ChewLengLeng",
    date: "Jun 15th 2022",
    image: ["https://www.kindmeal.my/photos/moment/24/24024-45786-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24039-45827-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24032-45800-m.jpg"

    ],
    likeimg: "https://www.kindmeal.my/images/icon_heart_darkgrey.svg",
    camara: "https://www.kindmeal.my/images/icon_camera_darkgrey.png",
    msg: "https://www.kindmeal.my/images/icon_comment_darkgrey.png",
    des: "小炒RM7.00起 Vegetarian stall at Restoran 2+1 素食档在贰加壹茶餐室 Add :",

},
{
    logo: "https://www.kindmeal.my/photos/member/11/11540-s.jpg",
    name: "ChewLengLeng",
    date: "Jun 14th 2022",
    image: ["https://www.kindmeal.my/photos/moment/24/24016-45752-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24039-45827-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24032-45800-m.jpg"
    ],
    socialLogo: "https://www.kindmeal.my/images/icon_facebook_moment.png",
    des: "小炒RM6.00起 Loves Veggie 爱素食 Add : Lot 10215, Ground Floor, Ja.. see more",
    link: "LOVES Veggie 愛素食（美里）",
    address: "LOT 10215 GRD FLR JALAN KAMPUNG PENGKALAN LUTONG, 98100 Miri",
    likeimg: "https://www.kindmeal.my/images/icon_heart_darkgrey.svg",
    camara: "https://www.kindmeal.my/images/icon_camera_darkgrey.png",
    msg: "https://www.kindmeal.my/images/icon_comment_darkgrey.png"

},
{
    logo: "https://www.kindmeal.my/photos/member/11/11812-s.jpg",
    name: "DavinaGoh",
    date: "Jun 14th 2022",
    image: ["https://www.kindmeal.my/photos/moment/24/24039-45827-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24042-45839-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24032-45800-m.jpg"
    ],
    socialLogo: "https://www.kindmeal.my/images/icon_facebook_moment.png",
    des: "Even though I share this regularly as a tip in my interviews.. see more",
    link: "Davina Da Vegan",
    likeimg: "https://www.kindmeal.my/images/icon_heart_darkgrey.svg",
    camara: "https://www.kindmeal.my/images/icon_camera_darkgrey.png",
    msg: "https://www.kindmeal.my/images/icon_comment_darkgrey.png"

},
{
    logo: "https://www.kindmeal.my/photos/member/19/19456-s.jpg",
    name: "William21",
    date: "Jun 13th 2022",
    image: ["https://www.kindmeal.my/photos/moment/24/24042-45839-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24032-45800-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24024-45787-m.jpg"
    ],
    socialLogo: "https://www.kindmeal.my/images/icon_facebook_moment.png",
    link: "Casa Of Italy",
    address: "27, Jalan SS20/11, Damansara KIM, 47400 Petaling Jaya",
    likeimg: "https://www.kindmeal.my/images/icon_heart_darkgrey.svg",
    camara: "https://www.kindmeal.my/images/icon_camera_darkgrey.png",
    msg: "https://www.kindmeal.my/images/icon_comment_darkgrey.png",
    des: "😋",

},
{
    logo: "https://www.kindmeal.my/photos/member/31/31747-s.jpg",
    name: "YeeHuanGan",
    date: "Jun 13th 2022",
    image: ["https://www.kindmeal.my/photos/moment/24/24032-45800-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24024-45787-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24027-45794-m.jpg"
    ],
    socialLogo: "https://www.kindmeal.my/images/icon_facebook_moment.png",
    link: "Good Food By Gan",

    likeimg: "https://www.kindmeal.my/images/icon_heart_darkgrey.svg",
    camara: "https://www.kindmeal.my/images/icon_camera_darkgrey.png",
    msg: "https://www.kindmeal.my/images/icon_comment_darkgrey.png",
    des: "[Recipe] Soft Tofu with Garlicky Sauce🧄 An easy recipe, rea.. see more",

},
{
    logo: "https://www.kindmeal.my/photos/member/35/35238-s.jpg",
    name: "CindyChang",
    date: "Jun 13th 2022",
    image: ["https://www.kindmeal.my/photos/moment/24/24024-45787-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24024-45787-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24027-45794-m.jpg"
    ],

    likeimg: "https://www.kindmeal.my/images/icon_heart_darkgrey.svg",
    camara: "https://www.kindmeal.my/images/icon_camera_darkgrey.png",
    msg: "https://www.kindmeal.my/images/icon_comment_darkgrey.png",
    des: "早餐在怡保望里万已刹早上素食档,谢谢好友帶我去吃一餐簡簡單單的早餐,可是很美味都是小时候的味道,幸福的感觉🙏😆 see more",

},
{
    logo: "https://www.kindmeal.my/photos/member/14/14169-s.jpg",
    name: "MayNg",
    date: "Jun 13th 2022",
    image: ["https://www.kindmeal.my/photos/moment/24/24024-45787-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24027-45794-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24041-45833-m.jpg"
    ],

    likeimg: "https://www.kindmeal.my/images/icon_heart_darkgrey.svg",
    camara: "https://www.kindmeal.my/images/icon_camera_darkgrey.png",
    msg: "https://www.kindmeal.my/images/icon_comment_darkgrey.png",
    des: "Happy Vegetarian in Johor Bahru! Nothing beats having the best Ingots Dumplings or Wantons (some called it) in JB! Love it when e.. see more",

},
{
    logo: "https://www.kindmeal.my/photos/member/23/23028-s.jpg",
    name: "YinYinBoey",
    date: "Jun 12th 2022",
    image: ["https://www.kindmeal.my/photos/moment/24/24027-45794-m.jpg",

        "https://www.kindmeal.my/photos/moment/24/24041-45833-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24018-45760-m.jpg"

    ],
    socialLogo: "https://www.kindmeal.my/images/icon_facebook_moment.png",
    link: "Raw Chef Yin",
    address: "Malaysia",
    likeimg: "https://www.kindmeal.my/images/icon_heart_darkgrey.svg",
    camara: "https://www.kindmeal.my/images/icon_camera_darkgrey.png",
    msg: "https://www.kindmeal.my/images/icon_comment_darkgrey.png",
    des: "Yesterday’s salad! Very surprised how creamy the dressing tu.. see more",

},
{
    logo: "https://www.kindmeal.my/photos/member/0/1-s.jpg",
    name: "FatJedi",
    date: "Jun 12th 2022",
    image: ["https://www.kindmeal.my/photos/moment/24/24041-45833-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24018-45760-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24027-45794-m.jpg"

    ],
    socialLogo: "https://www.kindmeal.my/images/icon_facebook_moment.png",
    link: "Bröom Artisan Bakery",
    address: "27 Jalan 20/13, Taman Paramount, 46300 Petaling Jaya",
    likeimg: "https://www.kindmeal.my/images/icon_heart_darkgrey.svg",
    camara: "https://www.kindmeal.my/images/icon_camera_darkgrey.png",
    msg: "https://www.kindmeal.my/images/icon_comment_darkgrey.png",
    des: "Light lunch of creative bakery. Tried the appetizing Kimchi Danish topped with runny egg yolk; and a matcha white chocolate hazeln.. see more",

},
{
    logo: "https://www.kindmeal.my/photos/member/22/22038-s.jpg",
    name: "JasonFong",
    date: "Jun 12th 2022",
    image: ["https://www.kindmeal.my/photos/moment/24/24018-45760-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24027-45794-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24041-45833-m.jpg"
    ],
    socialLogo: "https://www.kindmeal.my/images/icon_facebook_moment.png",
    link: "Tanoshi Mart",
    address: "10, Jalan Radin Bagus 6, Bandar Baru Sri Petaling, 57000 Kuala Lumpur,..",
    likeimg: "https://www.kindmeal.my/images/icon_heart_darkgrey.svg",
    camara: "https://www.kindmeal.my/images/icon_camera_darkgrey.png",
    msg: "https://www.kindmeal.my/images/icon_comment_darkgrey.png",
    des: "今天來 Tanoshi Mart 買無酒精紅酒🍷，遇到我的birthday month, 剛好這裡會員有送壽星一杯辣辣杯.. see more"

},
{
    logo: "https://www.kindmeal.my/photos/member/22/22038-s.jpg",
    name: "JasonFong",
    date: "Jun 12th 2022",
    image: ["https://www.kindmeal.my/photos/moment/24/24018-45760-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24027-45794-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24041-45833-m.jpg"
    ],
    socialLogo: "https://www.kindmeal.my/images/icon_facebook_moment.png",
    link: "Tanoshi Mart",
    address: "10, Jalan Radin Bagus 6, Bandar Baru Sri Petaling, 57000 Kuala Lumpur,..",
    likeimg: "https://www.kindmeal.my/images/icon_heart_darkgrey.svg",
    camara: "https://www.kindmeal.my/images/icon_camera_darkgrey.png",
    msg: "https://www.kindmeal.my/images/icon_comment_darkgrey.png",
    des: "今天來 Tanoshi Mart 買無酒精紅酒🍷，遇到我的birthday month, 剛好這裡會員有送壽星一杯辣辣杯.. see more"

},
{
    logo: "https://www.kindmeal.my/photos/member/22/22038-s.jpg",
    name: "JasonFong",
    date: "Jun 12th 2022",
    image: ["https://www.kindmeal.my/photos/moment/24/24018-45760-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24027-45794-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24041-45833-m.jpg"
    ],
    socialLogo: "https://www.kindmeal.my/images/icon_facebook_moment.png",
    link: "Tanoshi Mart",
    address: "10, Jalan Radin Bagus 6, Bandar Baru Sri Petaling, 57000 Kuala Lumpur,..",
    likeimg: "https://www.kindmeal.my/images/icon_heart_darkgrey.svg",
    camara: "https://www.kindmeal.my/images/icon_camera_darkgrey.png",
    msg: "https://www.kindmeal.my/images/icon_comment_darkgrey.png",
    des: "今天來 Tanoshi Mart 買無酒精紅酒🍷，遇到我的birthday month, 剛好這裡會員有送壽星一杯辣辣杯.. see more"

},
{
    logo: "https://www.kindmeal.my/photos/member/35/35238-s.jpg",
    name: "CindyChang",
    date: "Jun 13th 2022",
    image: ["https://www.kindmeal.my/photos/moment/24/24024-45787-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24024-45787-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24027-45794-m.jpg"
    ],

    likeimg: "https://www.kindmeal.my/images/icon_heart_darkgrey.svg",
    camara: "https://www.kindmeal.my/images/icon_camera_darkgrey.png",
    msg: "https://www.kindmeal.my/images/icon_comment_darkgrey.png",
    des: "早餐在怡保望里万已刹早上素食档,谢谢好友帶我去吃一餐簡簡單單的早餐,可是很美味都是小时候的味道,幸福的感觉🙏😆 see more",

},
{
    logo: "https://www.kindmeal.my/photos/member/14/14169-s.jpg",
    name: "MayNg",
    date: "Jun 13th 2022",
    image: ["https://www.kindmeal.my/photos/moment/24/24024-45787-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24027-45794-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24041-45833-m.jpg"
    ],

    likeimg: "https://www.kindmeal.my/images/icon_heart_darkgrey.svg",
    camara: "https://www.kindmeal.my/images/icon_camera_darkgrey.png",
    msg: "https://www.kindmeal.my/images/icon_comment_darkgrey.png",
    des: "Happy Vegetarian in Johor Bahru! Nothing beats having the best Ingots Dumplings or Wantons (some called it) in JB! Love it when e.. see more",

},
{
    logo: "https://www.kindmeal.my/photos/member/25/25560-s.jpg",
    name: "LimKengTiong",
    date: "Jun 14th 2022",
    image: ["https://www.kindmeal.my/photos/moment/24/24024-45787-m.jpg",

        "https://www.kindmeal.my/photos/moment/24/24024-45786-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24039-45827-m.jpg"

    ],
    socialLogo: "https://www.kindmeal.my/images/icon_facebook_moment.png",
    link: "GEKIL",
    address: "no. 49-g jalan pju 5/20 the strand, 47810 Petaling Jaya",
    likeimg: "https://www.kindmeal.my/images/icon_heart_darkgrey.svg",
    camara: "https://www.kindmeal.my/images/icon_camera_darkgrey.png",
    msg: "https://www.kindmeal.my/images/icon_comment_darkgrey.png",
    des: "鸳鸯火锅（麻辣+番茄汤底） 北京烤鸭",

},
{
    logo: "https://www.kindmeal.my/photos/member/11/11540-s.jpg",
    name: "ChewLengLeng",
    date: "Jun 15th 2022",
    image: ["https://www.kindmeal.my/photos/moment/24/24024-45786-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24039-45827-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24032-45800-m.jpg"

    ],
    likeimg: "https://www.kindmeal.my/images/icon_heart_darkgrey.svg",
    camara: "https://www.kindmeal.my/images/icon_camera_darkgrey.png",
    msg: "https://www.kindmeal.my/images/icon_comment_darkgrey.png",
    des: "小炒RM7.00起 Vegetarian stall at Restoran 2+1 素食档在贰加壹茶餐室 Add :",

},
{
    logo: "https://www.kindmeal.my/photos/member/11/11540-s.jpg",
    name: "ChewLengLeng",
    date: "Jun 14th 2022",
    image: ["https://www.kindmeal.my/photos/moment/24/24016-45752-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24039-45827-m.jpg",
        "https://www.kindmeal.my/photos/moment/24/24032-45800-m.jpg"
    ],
    socialLogo: "https://www.kindmeal.my/images/icon_facebook_moment.png",
    des: "小炒RM6.00起 Loves Veggie 爱素食 Add : Lot 10215, Ground Floor, Ja.. see more",
    link: "LOVES Veggie 愛素食（美里）",
    address: "LOT 10215 GRD FLR JALAN KAMPUNG PENGKALAN LUTONG, 98100 Miri",
    likeimg: "https://www.kindmeal.my/images/icon_heart_darkgrey.svg",
    camara: "https://www.kindmeal.my/images/icon_camera_darkgrey.png",
    msg: "https://www.kindmeal.my/images/icon_comment_darkgrey.png"

}
]

let content = document.getElementById("content");

let pagination_element = document.getElementById("pagination");

let current_page = 1;
let rows = 12;

displayProduct(product, content, rows, current_page);

function displayProduct(product, content, rows_per_page, page) {
    content.innerHTML = "";
    page--;
    let start = page * rows;
    let end = start + rows_per_page;

    let filtered = product.slice(start, end)
    filtered.map(function (ele) {
        let card = document.createElement("div");
        let box = document.createElement("div");
        // div1
        div1 = document.createElement("div");

        let logo = document.createElement("img");
        logo.src = ele.logo;


        let hbody = document.createElement("div");
        let name = document.createElement("p")
        name.innerText = ele.name;
        let date = document.createElement("p")
        date.innerText = ele.date;
        hbody.append(name, date)

        let btn = document.createElement("button");
        btn.innerText = "View"
        div1.append(logo, hbody, btn)
        //div2
        // div2 = document.createElement("div");

        let counter = 0;

        slideShow();
        function slideShow() {


            if (counter < ele.image.length) {
                let image = ele.image[counter];

                box.innerHTML = `<img src=${image}>`;
                counter++;
            } else {
                counter = 0;
            }
            setTimeout(slideShow, 2000);
        }

        //div3
        div3 = document.createElement("div");
        let likeImg = document.createElement("img");
        likeImg.src = ele.likeimg;
        let camara = document.createElement("img");
        camara.src = ele.camara
        let message = document.createElement("img");
        message.src = ele.msg;
        div3.append(likeImg, camara, message);
        //div4
        div4 = document.createElement("div");
        let para = document.createElement("p");
        para.innerText = ele.des;

        div4.append(para)

        //div5

        div5 = document.createElement("div");

        let socialimg = document.createElement("img");
        socialimg.src = ele.socialLogo;

        //inner div
        divinner = document.createElement("div");
        let p1 = document.createElement("p");
        let p2 = document.createElement("p");

        if (ele.address !== undefined && ele.link !== undefined) {
            p1.innerText = ele.link;
            p2.innerText = ele.address
            divinner.append(p1, p2)
        }
        else if (ele.address !== undefined) {
            p1.innerText = ele.address;
            divinner.append(p1)
        }

        else if (ele.link !== undefined) {
            p2.innerText = ele.link
            divinner.append(p2)
        }



        if (ele.socialLogo !== undefined) {
            div5.append(socialimg, divinner);
        } else {
            div5.append(divinner);
        }

        card.append(div1, box, div3, div4, div5)
        document.querySelector("#content").append(card);
    })
}


function setupPagination(product, wrapper, rows_per_page) {
    wrapper.innerHTML = "";
    let page_count = Math.ceil(product.length / rows_per_page);
    for (let i = 1; i <= page_count; i++) {
        let btn = paginationButton(i, product);
        wrapper.appendChild(btn);
    }
}
function paginationButton(page, product) {
    let button = document.createElement("button");
    button.innerText = page;
    if (current_page == page) button.classList.add("active");
    button.addEventListener("click", function () {
        current_page = page;
        displayProduct(product, content, rows, current_page);
        let current_btn = document.querySelector(".pagenumbers button.active")
        current_btn.classList.remove("active");
        button.classList.add("active");
    })

    return button;
}

setupPagination(product, pagination_element, rows);
